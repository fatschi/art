...
/**
 * @author Nils Rethmeier Linear classifier that is used to map a high
 *         dimensional feature vector into a lower dimensional (1 Bit
 *         dimensional) representation by classifying it as either 0 or 1. The
 *         classifiers weight vector w_i is chosen at random from a pseudo
 *         Normal Distribution with mean=0 and variance 1 for each individual
 *         weight. The linear decision function is f(xi) = wi * xi; where xi is
 *         a high dimensional feature vector. The classifier is sign(f(x)) = { 1
 *         if f(xi) > 0; 0 else
 */
public class RandomProjection {

	/**
	 * defines the # of random hyperplanes as well as the lenght of the
	 * resulting lower dimensional feature vector.
	 */
	private int d = 0;
	private int k = 0;
	 ...
	 * @param mean
	 * @param variance
	 * @return a float Very Sparse Random Projections "Achlioptas proposed
	 *         sparse random projections by replacing the N (0, 1) entries 2 in
	 ...
	 * 
	 * @param x
	 *            - the input feature vector of counts.
